...
- (void)startTaskWithServicePath:(NSString*)path httpMethod:(NSString *)method params:(NSDictionary*)params success:(BSUWebServiceClientSuccessBlock)success fail:(BSUWebServiceClientFailBlock)fail {
    ...
    if (error && fail) {
        fail([BSUErrorStatus errorWith:error]);
    }
    ...
    if (!task || params) {
        ...
        NSURLSessionDataTask *dataTask = [manager dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
            ...
            if (error) {
                ...
                if (fail) fail([BSUErrorStatus errorWith:error]);
            } else {
...
//        fail([BSUErrorStatus errorWithCode:BSUErrorStatusCodeTaskAlreadyRunning userInfo:@{NSURLErrorFailingURLStringErrorKey: urlString}]);
    }
}
